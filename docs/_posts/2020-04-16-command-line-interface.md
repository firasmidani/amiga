...
Accepts an integer and used to define how many time points to include in hypothesis testing. 1 indicates that each time point is included (i.e. T0, T1, T2, ...); 2 indicates that every other time point is included beginning with the first one (i.e. T0, T2, T4, ...); and so on. The default value is 11 (T0, T11, T22,...). This thinning of the data helps speed up the GP inference and reduction of the input to the GP model does not drastically alter model predictions as long as the input growth curve is smooth and does not exhibit acute spikes or dips. 
...
Accepts comma-separated list of variables in mapping files that will be used to specify which wells are control samples. See [Normalizing Parameters](/amiga/doc/normalization.html) for more details.

**\-\-pool-by**

Accepts comma-separated list of variables in mapping files that will be used to specify how samples will be grouped See [Pooled Modelling](/amiga/doc/pooling.html) for more details.
