...
To fit the growth curves, you run the following command. The `--merge-summary` argument will report the results in a single text file with the name passed with the `-o` argument `Biolog_ERandomii`. Fitting growth curves takes about 1-3 minutes per 96-well plate, depending on the complexity of the curves. The full analysis will take about 15 minutes. You added the `--verbose` argument so that `AMiGA` shares in real time more details about the input, data processing, and results.

```bash
python main.py -i /home/outbreaks/erandomii --normalize-parameters --merge-summary -o 'Biolog_ERandomii' --verbose
```
