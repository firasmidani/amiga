...
```bash
python amiga.py -i /home/outbreaks/erandomii --merge-summary
```
The files in the `summary` folder have been updated to include a single summary file with a name includes a time tamp `summary_{Year}-{Month}-{Day}_{Hour}-{Minutes}-{Seconds}.txt`. Because we invoked the `--merge-summary`, the results were merged into a ingle file. If we did not invoke the argument, `AMiGA` would have upgraded all of the files previously in the `summary` folder to include microbial growth parameters.
